...
-   **Probability theory** is a mathematical framework for inductive reasoning.
...
A probabilistic model consists of three components: the sample space $\Omega$, the event space $F$ and the probability measure/distribution/law $P$.

The **sample space** $\Omega$ is the set of outcomes of the random phenomenon/stochastic process/"experiment" being modeled. It must be mutually exclusive and collectively exhaustive and (art) at the right "granularity".
...
The probability measure/distribution/law $P$ is a function from the event space $F$ to the real numbers $\mathbb{R}$:
...
Probability as a function $P : F \rightarrow \mathbb{R}$ must satisfy a number of axioms (Kolmogorov axioms; see also [Cox's theorem](https://en.wikipedia.org/wiki/Cox%27s_theorem) for an alternative formulation):
