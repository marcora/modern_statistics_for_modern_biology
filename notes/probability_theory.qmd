...
Probability is a function $P$ that assigns a number between 0 and 1 to each event/subset of the sample space $\Omega$, i.e., the non-empty set of all possible outcomes of a random phenomemon/process/"experiment", e.g., tossing a coin.

$P : E \rightarrow [0,1]$

This function $P$ must satisfy a number of axioms (Kolmogorov axioms or Cox's theorem):

-   $P(A) \ge 0$
-   $P(\Omega) = 1$
-   $P$ is countably additive: if $\{A_i\}_{i=1}^n \subseteq E$ is a countable collection of pairwise disjoint sets, then $P(\cup_{i=1}^n A_i) = \sum_{i=1}^n P(A_i)$.
